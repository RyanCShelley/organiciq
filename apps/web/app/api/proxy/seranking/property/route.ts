...
export async function POST(request: Request) {
  ...
  if (!clientId || !body.site_id) {
    return NextResponse.json({ detail: "clientId and site_id required" }, { status: 400 });
  }

  const res = await fetch(`${API_URL}/integrations/seranking/property`, {
    method: "POST",
    headers: {
